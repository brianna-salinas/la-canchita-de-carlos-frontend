...
export default function SolicitudesAccesoPage() {
  ...
  return (
    <AppShell showSearch={false} minimalMobile>
      ...
      {/* ================= MOBILE ================= */}
      <div className="md:hidden px-4 py-4 pb-8 space-y-3 bg-neutral-50 min-h-screen">
        <p className="font-sans text-xs font-semibold text-neutral-500 uppercase tracking-wide">
          Solicitudes Pendientes
        </p>
        {pendientes.length === 0 && (
          <p className="font-sans text-sm text-neutral-400">No hay solicitudes pendientes.</p>
        )}
        {pendientes.map((s) => (
          <div key={s.id} className="bg-white rounded-2xl border border-neutral-200 p-4">
            <div className="flex items-start justify-between gap-2">
              <p className="font-sans font-bold text-base text-neutral-900">{s.nombre}</p>
              <span className="shrink-0 rounded-lg bg-neutral-100 px-2 py-1 font-sans text-[11px] font-medium text-neutral-500">
                {formatFechaRelativa(s.creadoEn)}
              </span>
            </div>
            <p className="font-sans text-sm text-neutral-500 mt-0.5">{s.correo}</p>
        ...
        <p className="font-sans text-xs font-semibold text-neutral-500 uppercase tracking-wide pt-3">
          Usuarios Aprobados
        </p>
        {usuariosAprobados.length === 0 && (
          <p className="font-sans text-sm text-neutral-400">Todavía no hay usuarios con acceso.</p>
        )}
        {usuariosAprobados.map((u) => (
          <div key={u.id} className="bg-white rounded-2xl border border-neutral-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="font-sans font-bold text-sm text-neutral-900">{u.nombre}</p>
                <p className="flex items-center gap-1.5 font-sans text-xs text-neutral-400 mt-0.5">
                  <span
            ...
            <div className="space-y-2 mt-3">
              <button
                ...
                className="w-full h-10 rounded-lg border border-neutral-200 font-sans font-semibold text-sm text-neutral-700 flex items-center justify-center gap-1.5"
              >
      ...
      <div className="hidden md:block">
        ...
        <h1 className="font-sans font-bold text-3xl text-neutral-900 mt-2">Solicitudes de Acceso</h1>
        ...
        <div className="grid grid-cols-[220px_1fr] gap-6 mt-6">
          ...
          <div>
            <div className="flex items-center gap-6 border-b border-neutral-200">
              ...
              >
                Pendientes
                <span className="h-5 min-w-[20px] px-1.5 rounded-full bg-neutral-100 text-neutral-500 text-[11px] font-bold flex items-center justify-center">
                  {String(pendientes.length).padStart(2, '0')}
                </span>
              </button>
              <button
            ...
            <div className="space-y-4 mt-5">
              ...
              {tab === 'aprobados' && aprobadas.length === 0 && (
                <p className="font-sans text-sm text-neutral-400">Todavía no hay solicitudes aprobadas.</p>
              )}
              {(tab === 'pendientes' ? pendientes : aprobadas).map((s) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-sans font-bold text-sm text-neutral-900">{s.nombre}</p>
                  ...
                  {tab === 'pendientes' && (
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button
                        onClick={() => rechazar(s.id)}
                        className="h-9 px-4 rounded-lg border border-neutral-200 font-sans font-semibold text-sm text-neutral-600 hover:bg-neutral-50"
                      >
                        Rechazar
                      </button>
                    </div>
        ...
        <div className="flex items-center justify-between mt-8 mb-4">
          <h2 className="font-sans font-bold text-lg text-neutral-900">Usuarios Aprobados</h2>
        ...
        <div className="space-y-4">
          ...
          {usuariosAprobados.map((u) => (
            <div key={u.id} className="bg-white rounded-2xl border border-neutral-200 p-4 flex items-center gap-4">
              <span className="h-11 w-11 rounded-full bg-brand-secondary/25 text-brand-primary font-sans text-sm font-bold flex items-center justify-center shrink-0">
                {iniciales(u.nombre)}
              </span>
              <div className="flex-1 min-w-0">
